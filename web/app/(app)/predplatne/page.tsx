...
export default function PricingPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-bold text-white"><CreditCard size={20} className="text-[#9184d9]" /> Předplatné</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 max-w-3xl">
        ...
        <PlanCard name="Pro" price="brzy" features={PRO} accent current={plan === "pro"}
          cta={user ? <button disabled className="w-full min-h-10 rounded-lg border border-[#5d5294] bg-[#2b2741] text-sm font-medium text-[#d2cefd] opacity-70 cursor-not-allowed">Předplatit (brzy)</button>
                    : <Link href="/registrace" className="block w-full min-h-10 rounded-lg border border-[#5d5294] bg-[#2b2741] text-center leading-10 text-sm font-medium text-[#d2cefd] hover:bg-[#423a6a]">Vytvořit účet</Link>} />
      </div>
...
function PlanCard({ name, price, features, current, accent, cta }: {
  ...
  return (
    <div className={`rounded-2xl border p-5 ${accent ? "border-[#5d5294] bg-[#1b1830]" : "border-[#2a2d3a] bg-[#151823]"}`}>
      ...
      <ul className="mt-4 space-y-2">
        {features.map((f) => (
          <li key={f} className="flex items-start gap-2 text-sm text-gray-300"><Check size={15} className="mt-0.5 shrink-0 text-[#9184d9]" /> {f}</li>
        ))}
      </ul>
